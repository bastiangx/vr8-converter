<script lang="ts">
  ...
  $: shortPath = outputDir.split('/').slice(-3).join('/');
...
<div class="card bg-base-200 shadow-xl w-full max-w-lg mx-auto text-center">
  <div class="card-body">
    ...
    <p class="text-lg">📁 Output folder: {shortPath}</p>

    <div class="card-actions justify-center mt-4 gap-4">
      <button class="btn btn-secondary" on:click={onConvertMore}>
        Convert More
      </button>
      <button class="btn btn-wide btn-primary" on:click={openFolder}>
        Open Folder
      </button>
    </div>
  </div>
